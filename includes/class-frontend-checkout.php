...
class Frontend_Checkout {
    ...
    public function __construct() {
        // Remove the hook to avoid duplicate forms with Divi
        // add_action('woocommerce_before_checkout_form', array($this, 'display_delivery_form'));
    ...
    public function add_delivery_fee_to_cart($cart) {
        error_log('[TimeFlow Debug][Fee Calculation] Running add_delivery_fee_to_cart.');
        if (is_admin() && !defined('DOING_AJAX')) {
            // error_log('[TimeFlow Debug][Fee Calculation] Exiting: Is admin and not AJAX.');
            return;
        }

        // Check if WC(), session and cart available
        if (!function_exists('WC') || !WC()->session || !$cart || !method_exists($cart, 'fees_api')) {
             error_log('[TimeFlow Debug][Fee Calculation] Exiting: WC, session or cart fees_api not available.');
            return;
        }
        
        $delivery_type = WC()->session->get('timeflow_delivery_type');
        $time_slot_id = WC()->session->get('timeflow_time_slot_id');
        $time_slot_fee = WC()->session->get('timeflow_time_slot_fee'); // Use the dedicated fee variable

        error_log('[TimeFlow Debug][Fee Calculation] Session Data - Type: ' . print_r($delivery_type, true) . ', Slot ID: ' . print_r($time_slot_id, true) . ', Fee: ' . print_r($time_slot_fee, true));
        
        $fee_name = __('Delivery Fee', 'woocommerce-timeflow-delivery');
        $should_add_fee = false; // Default to false
        if ( !empty($time_slot_id) && $time_slot_fee !== '' && is_numeric($time_slot_fee) && (float)$time_slot_fee > 0 ) {
            // A valid time slot with a fee > 0 is selected
            if ($delivery_type === 'shipping') {
                $should_add_fee = true;
            } elseif ($delivery_type === 'pickup') {
                $should_add_fee = true; // ALLOW FEE FOR PICKUP
            }
        }
        
        // Use Fees API to add or remove the fee
        $fees_api = $cart->fees_api();
        $existing_fees = $fees_api->get_fees();

        if ($should_add_fee) { // Use the combined flag
            error_log('[TimeFlow Debug][Fee Calculation] Adding/Updating Fee: ' . $time_slot_fee . ' for type: ' . $delivery_type);
            $fees_api->add_fee(array(
                'id'        => 'timeflow_delivery_fee', // Unique ID for the fee
                'name'      => $fee_name,
                'amount'    => (float) $time_slot_fee,
                'taxable'   => false, // Adjust if taxable
                'tax_class' => ''
            ));
        } else {
            // Check if our specific fee exists and remove it if conditions are not met
            if (isset($existing_fees['timeflow_delivery_fee'])) {
                 error_log('[TimeFlow Debug][Fee Calculation] Conditions not met. Removing existing Delivery Fee.');
                 $fees_api->remove_fee('timeflow_delivery_fee');
            } else {
                 error_log('[TimeFlow Debug][Fee Calculation] Conditions not met and no existing fee found.');
            }
        }
    }
    ...
    public function display_shipping_details_on_review() {
        ...
        if ($delivery_type === 'shipping') {
            ...
            $label = __('Shipping', $text_domain); // Simple label for shipping
           
        } elseif ($delivery_type === 'pickup') {
             // error_log('[TimeFlow Debug][Display Details] Delivery type is pickup.');
             $label = __('Local Pickup', $text_domain); // Changed text
        ...
        if (!empty($label)) {
             ...
             $details_html .= '<th scope="row">' . __('Delivery:', $text_domain) . '</th>'; 
